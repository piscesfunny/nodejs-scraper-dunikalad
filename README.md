...
`getListings` is used to scrape a list of results on a given website.getListings
...
I don't care how you get the data as long as the final result from `scrapePage` passes the `isValidItem` test function.
...
Create a new file from the template file in the same directory as the template file and name it after the website in pascal case. Using the example above, we would name it `Dpdhl`


## Define the `getListings` function

You can test it by running `npm test-listing`. To see how this is used see `test.js`. You need to set the `source` env variable which will tell it which source to test. Using the example above it would be `source=Dpdhl npm test-listing`.
...
You can test it by running `npm test-page`. To see how this is used see `test.js`. You need to set the `source` env variable which will tell it which source to test. You also need to set the `url` environment variable to tell it which page to scrape.

Using the example above it would be `url='https://www.dpdhl.jobs/job/dublin/general-operative/1886/12541176' source=Dpdhl npm test-page`.
